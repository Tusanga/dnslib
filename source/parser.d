...
dnsParserResult dnsParse(const ref ubyte[] input, ref dnsMessage myDnsMessage, bool printEnabled = false)
{
  ...
  try
  {
    ...
    void parseResponse()
    {
      ...
      // The resource record (RR) fields are name, then 10 bytes of fixed fields incl. the responseDataLength and finally the responseData itself
      enum RECORD_FIXED_FIELDS_SIZE = 10;
      if (input.length - inputPtr < RECORD_FIXED_FIELDS_SIZE) { throw new dnsParserException(dnsParserResult.dataMissing); }
      ...
      inputPtr += RECORD_FIXED_FIELDS_SIZE;
    ...
    foreach (i; 0 .. myDnsMessage.header.QDCOUNT)
    {
      ...
      if (printEnabled) writefln("Parsing question records %d: %d / %d", i, inputPtr, input.length);
      parseQuery();
    ...
    foreach (i; 0 .. myDnsMessage.header.ANCOUNT)
    {
      ...
      if (printEnabled) writefln("Parsing answer records %d: %d / %d", i, inputPtr, input.length);
      parseResponse();
    ...
    {
      myResponseSection = responseSection.init;
      if (printEnabled) writefln("Parsing authoritative records %d: %d / %d", i, inputPtr, input.length);
      parseResponse();
    ...
    {
      myResponseSection = responseSection.init;
      if (printEnabled) writefln("Parsing additional records %d: %d / %d", i, inputPtr, input.length);
      parseResponse();
      myDnsMessage.additional ~= myResponseSection;
    }

    if (printEnabled) writefln("Parsing done: %d / %d", inputPtr, input.length);
    assert(input.length >= inputPtr);
